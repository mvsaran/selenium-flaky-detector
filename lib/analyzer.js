...
const STABLE_RCA = {
    id: 'stable',
    label: 'Reliable (Consistent Pass)',
    emoji: '✅',
    description: 'Test is consistently passing across all runs. No fix required.',
    fix: null
};
...
const RCA_PATTERNS = [
    ...
    // ── 4. Async text load (status bar says "Fetching..." instead of "Loaded") ──
    {
        ...
        patterns: [
            /to contain.*"Loaded"/i,
            /to contain.*"Fetching"/i,
            /to contain.*"Loading"/i,
            /Expected condition failed/i,
            /waiting for presence/i,
            /waiting for visibility/i,
        ],
...

    {
        ...
        description: 'A general assertion failure occured. If this is intermittent, it is likely a race condition or state inconsistency.',
        fix: {
            title: 'Harden Assertions with Explicit Waits',
            before:
                '// ❌ Problem: Immediate assertion without waiting for state\n' +
                'assertTrue(page.isLoaded());',
            after:
                '// ✅ Fix: Use WebDriverWait to ensure state before asserting\n' +
...

    {
        ...
        description: 'Test encountered a NullPointerException. Often caused by failed initialization or race conditions in test setup.',
        fix: {
            title: 'Add Defensive Null Checks',
            before:
                '// ❌ Problem: Direct access to potentially null object\n' +
                'String text = driver.getTitle();\n' +
                'int len = text.length();',
            after:
                '// ✅ Fix: Verify object exists before access\n' +
                'String text = driver.getTitle();\n' +
                'if (text != null) {\n' +
                '    int len = text.length();\n' +
                '}',
    ...
    {
        id: 'class_cast',
        label: 'Class Cast Error',
        emoji: '🛡️',
        patterns: [
            /ClassCastException/i,
            /cannot be cast to/i,
        ],
        description: 'A type mismatch occured, often when trying to cast a generic WebElement to a specific proxy or decorated type.',
        fix: {
            title: 'Verify Types Before Casting',
            before:
                '// ❌ Problem: Unsafe cast\n' +
                'MyElement el = (MyElement) driver.findElement(By.id("btn"));',
            after:
                '// ✅ Fix: Check type or use generic WebElement\n' +
                'WebElement raw = driver.findElement(By.id("btn"));\n' +
                'if (raw instanceof MyElement) {\n' +
                '    MyElement el = (MyElement) raw;\n' +
                '}',
        },
    },
...
class RootCauseAnalyzer {
    ...
    analyze(parsedRuns, scores) {
        ...
        for (const [testName, scoreData] of Object.entries(scores)) {
            ...
            let rca;
            if (scoreData.isAlwaysPass) {
                rca = STABLE_RCA;
            } else {
                rca = this._classify(allText);
            }
    ...
    _countByRca(perTest) {
        ...
        for (const data of Object.values(perTest)) {
            if (data.flakinessScore > 0 || data.isAlwaysFail) {
    ...
    _topRootCauses(perTest) {
        ...
        const allPossibleRcas = [STABLE_RCA, UNKNOWN_RCA, ...RCA_PATTERNS];
        return Object.entries(counts)
            .sort((a, b) => b[1] - a[1])
            .map(([label, count]) => {
                const rca = allPossibleRcas.find(p => p.label === label);
